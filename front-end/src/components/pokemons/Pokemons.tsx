...
export const Pokemons = () => {
  ...
  useEffect(() => {
    const elementsinDatabease = 2400;
    dispatch({ type: PageManagerActions.SETLENGTH, payload: elementsinDatabease });
    let ar = Array.apply(null, Array(elementsinDatabease)).map((el, i) => {
      return { name: `twoja mama ${i}` };
    });
    ar = ar.slice(
      (pageManager.selectedPage - 1) * 40,
      pageManager.selectedPage * 40 > elementsinDatabease ? elementsinDatabease : pageManager.selectedPage * 40 + 1
    );
